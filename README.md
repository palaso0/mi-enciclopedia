# Códice

Archivo personal de aprendizaje y notas.

## Qué es

Un sitio web estático que funciona como enciclopedia personal de apuntes. Cada tema está explicado con analogías, diagramas visuales en CSS, ejemplos de código y tablas comparativas, pensado para aprender desde cero sin fórmulas crípetas.

## Cómo funciona

- **Hub principal** con tarjetas filtrables y búsqueda en tiempo real
- **Temas agrupados por categorías** como rutas de aprendizaje (cada categoría es una página con subtemas numerados)
- **Navegación secuencial** con botón "Siguiente tema" para seguir un orden de estudio
- **Modo oscuro** automático según preferencias del sistema
- **Animaciones de scroll** con IntersectionObserver
